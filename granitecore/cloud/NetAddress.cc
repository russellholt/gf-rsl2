 ...
 *
 * $Id: NetAddress.cc,v 1.2 1998/11/23 19:22:11 cking Exp $
 */
...
NetAddress::NetAddress (const RWCString& addrStr)
{
  ...
  if (host.isNull ())
  {
    throw RWCString ("IllegalArgumentException: Invalid host name");
  }

  RWCString portStr = st (":");
  port = atoi (portStr.data ());
  cryptoSubsystem = NULL;
  if (portStr.isNull () || (port == 0))
  {
    throw RWCString ("IllegalArgumentException: Invalid port number");
  }
}
			     ...
			     const int timeout) const
{
    ...
    {
        throw RWCString ("IOException: Can't establish connection");
    }
